...
<div class="admin__container-slider__navbar__content">
    <div class="admin__container-slider__navbar__content-form">
        ...
        <div class="admin__container-slider__navbar__content-form-wrap">
          <form action="" method="post" enctype="multipart/form-data" class="admin__form">
            <span class="admin__container-slider__navbar__content-form__text">Ảnh</span>
            <input type="file" name="image_banner" class="admin__container-slider__navbar__content-form__input">
            @error('image_banner')
            <span class="erros">{{ $message }}</span>
            @enderror
            <span class="admin__container-slider__navbar__content-form__text">Link</span>
            <input type="text" name="link_banner" id="" placeholder="Nhập link ở đây" class="admin__container-slider__navbar__content-form__input">
    ...
    <table class="admin__container-slider__navbar__content-table">
      ...
      @if(!empty($listSliderBanner))
      @foreach($listSliderBanner as $key => $items)
      <tr>
        <td>{{ $key + 1 }}</td>
        <td><img src="{{ asset('/image/slider_banner/'.$items->image)}}" class="admin__container-slider__navbar__content-table__img" alt="GearVN"></td>
        <td>{{ $items->link }}</td>
        <td><a onclick="return confirm('Bạn có chắc chắn xóa không? ')" href="{{ route('slider.delete',['id' => $items->id]) }}" class="admin__link"><i class="fas fa-trash-alt admin__container-slider__navbar__content-table__icon"></i></a></td>
      </tr>
      @endforeach
      @else
      <tr> 
