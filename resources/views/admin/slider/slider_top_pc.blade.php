...
<div class="admin__container-slider__navbar__content">
  ...
  <table class="admin__container-slider__navbar__content-table">
      ...
      @foreach($listSliderTopPc as $key =>$item)
      <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ $item->product_name }}</td>
        <td><a onclick="return confirm('Bạn có chắc chắn xóa không? ')" href="{{ route('slider.delete-slider-top-pc',['id' => $item->id]) }}" class="admin__link"><i class="fas fa-trash-alt admin__container-slider__navbar__content-table__icon"></i></a></td>
      </tr>
     @endforeach
